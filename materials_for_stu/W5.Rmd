...
```{r}
set.seed(100)
friend_comm_multi <- leiden(graph = net182_friend_und)
```

Let's look at the multi-level community structure.

```{r}
friend_comm_multi$membership 
```

```{r}
str(friend_comm_multi)
```
...
```{r}
mems_mult_level1 <- cut_at(friend_comm_multi, no = 2) 
mems_mult_level2 <- cut_at(friend_comm_multi, no = 4) 
...
## \[OPTION\] ## Demographic Characteristics
