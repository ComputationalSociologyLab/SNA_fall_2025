...
It is also possible to set the colors within the function itself using a vertex.color argument: `plot(class_net, vertex.color = cols)`
...
There are a number of other options a researcher could explore if they wanted to continue tweaking their plot. See the following help files for more options: `?plot.igraph` `?igraph.plotting`

## [Option]Network Plots based on ggplot
...
## [Option]Dynamic Network Visualizations
