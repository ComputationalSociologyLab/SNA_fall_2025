...
-   apply from student relationship dataset
...
## Let's see if cohesion exists with vertex connectivity
...
-   Solve the questions below by utilizing community detection
    -   Do male and female students form separate communities?
    -   How do the detected communities influence overall network cohesion?
...
plot(net182_friend_und, layout = layout,
     vertex.color = membership(friend_comm_wt4), 
     edge.color = "light gray", vertex.size = 20, main = "Walktrap: 4 Steps")
plot(net182_friend_und, layout = layout,
     vertex.color = membership(friend_comm_wt3), edge.color = "light gray", vertex.size = 20, main = "Walktrap: 3 Steps")
```
