...
## Reading in Data 
- `EpiModel` specify 
  - the formation and breaking of network ties
  - the spread of an infection through the network
```{r message=F, warning=F}
library(EpiModel)
```

## Reading in Data 
- Use a faux ego network data drawn from an at-risk population of drug users(No exact edgelist)
...
- Specify a model that determines how ties form and break within the simulation
  - With tie formation formula (`edges`, `nodefactor("location")`, `nodematch("location")`)
...
- Calculate the term `nodefactor("location")`
  - The mean degree of ties connected with same type of location
  - The mean degree of the group * the number of people in that group
  
```{r}
# calculate the mean degree of the group
...
```{r}
# calculate the number of people in that group
...
## Specifying the Network Model
- Specify a model on how edges are dropped over time
```{r}
...
## Estimating the Network Model
- Check if the model is producing networks that match the target statistics
...
## Estimating the Network Model
```{r}
# "duration" : the duration of ties
# "dissolution" :the dissolution of tie
par(mfrow = c(1, 2))
...
```{r}
# "s" "i" or "r" for susceptible, infected or recovered
...
## Specifying the Epidemic Model
- Set options to control the simulation itself
...
```{r}
episim_baseline <- netsim(x = net_mod, #simulated network
                          init = initial_status_inputs, # initial status
                          param = input_to_episim, # param for infection and recovery
                          control = control_episim # sim option
                          ) 
```
